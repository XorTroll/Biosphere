...
DIST_LIBUNWIND := $(LIB_DEP_LIBUNWIND) \
	$(BIOSPHERE_HOME)/include/libunwind.h \
	$(BIOSPHERE_HOME)/include/__libunwind_config.h

$(BIOSPHERE_HOME)/include/%: $(SOURCE_ROOT)/libunwind/include/%
	install -d $(@D)
	install $< $@
...
LIBCXX_CMAKE_EXE_LINKER_FLAGS := "-Wl,-T$(BIOSPHERE_HOME)/utils/ld/link.ld -fuse-ld=lld -L$(BIOSPHERE_HOME)/lib -lc -lclang_rt.builtins-aarch64 -lpthread -lm"
...
$(BUILD_DIR)/libcxxabi/Makefile: $(DIST_LIBCXX) $(DIST_TRANSISTOR_SUPPORT)
	...
	cd $(@D); cmake -G "Unix Makefiles" $(realpath $(SOURCE_ROOT))/libcxxabi \
		...
		-DLIBCXXABI_LIBCXX_LIBRARY_PATH="$(BIOSPHERE_HOME)/lib" \
		-DLIBCXXABI_LIBCXX_INCLUDES="$(BIOSPHERE_HOME)/include" \
